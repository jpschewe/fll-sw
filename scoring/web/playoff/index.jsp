...
<body>
	...
	<p>
		...
		available from the Admin Index or by clicking <a
			href='<c:url value="/admin/tables.jsp"/>' target='_new'>here</a>.
	...
	<p>
		Scores for the head to head brackets are entered just like the scores
		for the seeding rounds using the <a
			href='<c:url value="/scoreEntry/select_team.jsp"/>' target='_new'>Score
			Entry</a> page. If a team leaves the tournament before head to head, they
		should be entered as a No Show on the Score Entry page. If a tie
		occurs in a match; print out the score sheets again and run the teams
		a second time. Then change the scores for the teams to be the new
		scores.
	</p>

	<ol>

		<li><a id='create-bracket' href="create_playoff_division.jsp">Create
				head to head bracket</a></li>
		...
		<li>
			<form name='initialize' action='StorePlayoffParameters' method='POST'>
				...
				to enable 3rd/4th place match<br> <input type='submit'
					id='initialize_brackets' value='Initialize Bracket'
					${init_disabled } /> <a
					href='javascript:display("InitializeBracketHelp")'>[help]</a>
				<div id='InitializeBracketHelp' class='help' style='display: none'>
					Initializing a head to head bracket allows it to be run. A head to
					head bracket cannot be initialized if any teams that are to compete
					...
					match is need if you want to know not only 1st and 2nd place in the
					bracket, but 3rd and 4th place as well. This will add a match with
					the two teams that lost in the semi-final matches.<a
		...
		<li>
			<form name='printable' action='scoregenbrackets.jsp' method='get'
				...
				<%-- bracket --%>
				Select Bracket: <select id='printable.division' name='division'>
				...
				<div id='ScoreGenBracketsHelp' class='help' style='display: none'>
					This page is used to print out score sheets for the specified head
					to head bracket. This page will automatically assign the matches to
					the tables specified in on the table assignment page. The assigned
					tables can be changed before printing if desired. <a
		...
		<li>
			<form name='admin' action='adminbrackets.jsp' method='get'
				...
				<div id='PrintableBracketsHelp' class='help' style='display: none'>
					This page is used to print out the bracket for registration or the
					emcee. The score sheet generation bracket page should be used to
					print out the first matches before this page is used to print the
					bracket. This will ensure that the assigned tables appears on the
					printed bracket. <a href='javascript:hide("PrintableBracketsHelp")'>[hide]</a>
				</div>
		...
		<li>
			...
			<form name="uninitialize_playoff" method="POST"
				...
				</select> <input type='submit' id='uninitialize_playoff-submit'
					value='Submit'
				...
				<div id='UninitializeBracketsHelp' class='help'
					...
					cases this link should not be used. It may be useful if a head to
					head bracket was initialized by mistake and a different one should
					be run first. Any scores that have been entered for this bracket
					will be deleted.<a
		...
		<li>
			<%-- finish bracket --%>
			<form name="finish_bracket" method="POST" action="FinishBracket">
				Select bracket to finish: <select id='finish-bracket' name='bracket'>
					<c:forEach items="${playoff_data.unfinishedBrackets }"
						var="bracket">
						<option value='${bracket}'>${bracket}</option>
					</c:forEach>
				</select> <input type='submit' id='finish_bracket-submit' value='Submit'
					onclick='return confirm("Are you absolutly sure you want to fill in dummy scores for the rest of this head to head bracket?")' />

				<a href='javascript:display("FinishBracketsHelp")'>[help]</a>
				<div id='FinishBracketsHelp' class='help' style='display: none'>
					This link is used to finish a head to head bracket. This can be
					used when you don't want to run to the end of the bracket.
					Minnesota does this when running their state head to head as they
					run 2 brackets down to 4 teams each and then run a final 8 team
					bracket.<a href='javascript:hide("FinishBracketsHelp")'>[hide]</a>
				</div>

			</form> <%-- end finish division --%>
		</li>
