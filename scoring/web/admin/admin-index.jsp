	...
	<ol>
		<li>
			<form id='uploadTeams' ACTION="<c:url value='/UploadSpreadsheet'/>"
				...
				limited feedback. It's suggested that you edit the input file before
				upload to contain only the teams for your tournament(s). <b>This
					removes any existing teams from the database along with their
					scores.</b> <a href='javascript:display("UploadTeamsHelp")'>[help]</a>
				<div id='UploadTeamsHelp' class='help' style='display: none'>
					Each column of the input file needs to matched against the required
					data for teams. This information includes: team number, team name,
					organization, initial tournament, division. The team number must be
					a number and is required. The other columns are not required, but
					are a good idea to include. You will be prompted to pick a column
					from your data file to match against each piece of team data that
					the software uses. You can select the same column for multiple
					pieces of data. <a href='javascript:hide("UploadTeamsHelp")'>[hide]</a>
				</div>

				<input type="file" size="32" id='teams_file' name="file"> <input
					type='hidden' name='uploadRedirect'
					value="<c:url value='/admin/UploadTeams'/>" /> <input
					type="submit" id='upload_teams' value="Upload">
			</form>
		</li>
		...
		<li><a href='<c:url value="tournaments.jsp"/>'>Add or Edit
			...
			<div id='EditTournamentHelp' class='help' style='display: none'>
				...
				created by team import step above, to assign tournament advancement
				(e.g. teams may advance from regional tournaments to the state
				tournament), or to create new tournaments.<br> <a
		...
		<li>
			<form id='currentTournament' action='SetCurrentTournament'
				method="post">
				Current Tournament: <select id='currentTournamentSelect'
					name='currentTournament'>
					<c:forEach items="${tournaments }" var="tournament">
						<c:choose>
							<c:when test="${tournament.tournamentID == currentTournamentID}">
								<option selected value='${tournament.tournamentID }'>${tournament.location}
				...
				</select> <input type='submit' name='change_tournament'
		...
		<li><a href='edit_event_division.jsp'> Assign event divisions
				to teams in current tournament</a>. <a
			...
			<div id='EventDivisionHelp' class='help' style='display: none'>
				...
				team groupings are not based solely on the division of the teams,
				e.g. you have 2 groups of teams that are all division 1, use this
				page to assign &ldquo;event divisions&rdquo; to divide your
				tournament&rsquo;s teams into the groups in which they will be
				competing.<br> <a href='javascript:hide("EventDivisionHelp")'>[hide]</a>
			</div></li>
		...
		<li>
			<form name='uploadSubjective'
				...
				Upload the datafile for subjective scores. <input type="file"
					size="32" name="subjectiveFile" /> <input type="submit"
					value="Upload" />
			</form>
		</li>
