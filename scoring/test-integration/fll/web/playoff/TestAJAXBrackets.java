...
public class TestAJAXBrackets extends SeleneseTestBase {
  ...
  public void testAJAXBracketsInFull() throws IOException, SAXException {
    ...
    for (int i = 1; i < 6; ++i) {
      IntegrationTestUtils.addTeam(selenium, i, ""+i, "htk", "mn", "1", GenerateDB.DUMMY_TOURNAMENT_NAME);
    }
    //init brackets
    IntegrationTestUtils.loadPage(selenium, TestUtils.URL_ROOT+"playoff");
    ...
    //table labels
    IntegrationTestUtils.loadPage(selenium, TestUtils.URL_ROOT+"admin/tables.jsp");
    ...
    //open brackets
    selenium.openWindow(TestUtils.URL_ROOT+"playoff/remoteControlBrackets.jsp?scroll=false", "brackets");
    selenium.waitForPopUp("brackets", IntegrationTestUtils.WAIT_FOR_PAGE_TIMEOUT);
    //open score entry
    selenium.openWindow(TestUtils.URL_ROOT+"scoreEntry/select_team.jsp", "scoreentry");
    ...
    PostMethodWebRequest tableLabels = new PostMethodWebRequest(TestUtils.URL_ROOT+"playoff/ScoresheetServlet");
    ...
    WebResponse wr = wc.getResponse(tableLabels);
    
    //check for a blue cell
    selenium.selectWindow("brackets"); //TODO: I can't find a way selenium is OK with that checks for a present CSS property, nor a string of JS selenium is OK with to check for the element.
    //selenium.waitForCondition("window.document.getElementsByClassName('table_assignment')[0].style.backgroundColor=='blue'", JS_EVAL_TIMEOUT); // > 1 element with a style attrib that contains the string 'blue'
    
    
    selenium.setSpeed("100"); //I slow down selenium for the AJAX functions as while they don't take that long, selenium spend a lot less
                              //time between entering data and checking for it than it can keep up with.
  ...
  private void enterScore(Selenium selenium, String team, int score) {
    selenium.selectWindow("scoreentry");
    selenium.select("xpath=//form[@name='selectTeam']//select[@name='TeamNumber']", "value="+team);
