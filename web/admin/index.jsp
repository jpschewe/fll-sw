...
<body>
	...
	<ol>
		<li>
			<form id='uploadTeams' ACTION="<c:url value='/UploadSpreadsheet'/>"
				...
				comma separated or an Excel file (xls and xlsx supported). <input
					type="file" size="32" id='teams_file' name="file"> <input
					type='hidden' name='uploadRedirect'
					value="<c:url value='/admin/UploadTeams'/>" /> <input
					type="submit" id='upload_teams' value="Upload"> <a
					href='javascript:display("UploadTeamsHelp")'>[help]</a>
				<div id='UploadTeamsHelp' class='help' style='display: none'>
					Each column of the input file needs to matched against the required
					data for teams. The column names are the first row of your data
					file. This information includes: team number, team name,
					organization, initial tournament, award group, judging group. The
					team number must be a number and is required. The other columns are
					not required, but are a good idea to include. You will be prompted
					to pick a column from your data file to match against each piece of
					team data that the software uses. You can select the same column
					for multiple pieces of data. <a
		...
		<li><a href='editTeam.jsp'>Add a team</a> <a
			...
			<div id='AddTeamHelp' class='help' style='display: none'>
				...
				where all of the teams aren't known in advance or a team shows up
				for a tournament at the last minute. Special care needs to be taken
				when adding a team to an already running tournament to ensure that
				they have a schedule for where to go when and that they get judged
				properly. In most cases this function should not be used at the
				tournament, but rather before the tournament.<a
					href='javascript:hide("AddTeamHelp")'>[hide]</a>
			</div></li>

		<li><a id='add-edit-tournaments'
			href='<c:url value="tournaments.jsp"/>'>Add or Edit Tournaments</a> <a
			...
			<div id='EditTournamentHelp' class='help' style='display: none'>
				...
				created by team upload step above, or to create new tournaments.<br>
				<a href='javascript:hide("EditTournamentHelp")'>[hide]</a>
			</div></li>


		<li><a href='DisplayTournamentAssignments'>Display Tournament
				Assignments</a> <a
			...
			<div id='DisplayTournamentAssignmentsHelp' class='help'
				...
				assigned to what tournament and what judging groups they are in.<br>
				<a href='javascript:hide("DisplayTournamentAssignmentsHelp")'>[hide]</a>
			</div></li>
		...
		<li><c:if test="${scheduleUploaded }">
			...
			<div id='ScheduleHelp' class='help' style='display: none'>
				...
				when displaying information in final reports. <a
					href='javascript:hide("ScheduleHelp")'>[hide]</a>
			</div>
			<form id='uploadSchedule'
				action='<c:url value="/schedule/UploadSchedule"/>' METHOD="POST"
				ENCTYPE="multipart/form-data">
				<input type="file" size="32" name="scheduleFile" /> <input
					id="upload-schedule" type="submit" value="Upload Schedule" />
			</form> <c:if test="${scheduleUploaded }">
				<!--  downloads for the schedule -->
				<ul>
					<li><a href="ScheduleByTeam" target="_new">Full schedule
							sorted by team</a></li>
					...
					<li><a href="PerformanceSchedule" target="_new">Performance
							Schedule</a></li>
					<li><a href="<c:url value='/admin/ScheduleAsCsv' />"
		...
		<li><a id='change-award-groups' href='edit_event_division.jsp'>Change
			...
			<div id='EventDivisionHelp' class='help' style='display: none'>
				This information is typically specified in the schedule data file.
				If it needs to be modified you can use this page to change the award
				groups for teams in the current tournament. <br> <a
		...
		<li><a href='edit_judging_groups.jsp'>Change judging groups
			...
			<div id='JudgingGroupHelp' class='help' style='display: none'>
				This information is typically specified in the schedule data file.
				If it needs to be modified you can use this page to change the
				judging groups for teams in the current tournament. <br> <a
	...
	<ol>
		...
		<li><c:if test="${tablesAssigned}">
			...
			scoresheet printing during head to head)</li>
		...
		<li><a id='remote-control' href='remoteControl.jsp'>Remote
				control of display</a></li>

		<li><h3>Subjective Scoring</h3>
			<ul>
				<li><a href='<c:url value="/subjective/index.html"/>'
		...
		<li>
			<form id='uploadTeamTournamentAssignments'
				...
				Upload CSV or Excel of teams and tournaments to assign them to <input
					type="file" size="32" name="file" /> <input type='hidden'
					name='uploadRedirect'
