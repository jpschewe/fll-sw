...
<body>
	...
	<div class='content'>
		...
		<form name='edit_tournament_parameters'
			id='edit_tournament_parameters' action='StoreTournamentParameters'
			method='POST'>
				...
				<c:choose>
					<c:when test="${numSeedingSoundsDisabled}">
						<i>This parameters cannot be changed once any performance
							scores have been entered.</i>
						<input type="text" name="seeding_rounds" id="seeding_rounds"
							value="${numSeedingRounds}" class="required digits" readonly />
					</c:when>
					<c:otherwise>
						...
						<input type="text" name="seeding_rounds" id="seeding_rounds"
							value="${numSeedingRounds}" class="required digits" />
					</c:otherwise>
				</c:choose>
			</div>
			...
			<div>
				...
				<c:choose>
					...
					<c:otherwise>
						<a href='javascript:display("RunningHeadToHeadHelp")'>[help]</a>
						<div id='RunningHeadToHeadHelp' class='help' style='display: none'>
							When this parameter is unset, then there are no head to head
							rounds. Instead there are extra performance runs that don't count
							for the final reports. <a
								href='javascript:hide("RunningHeadToHeadHelp")'>[hide]</a>
						</div>
						<input type="checkbox" name="running_head_to_head"
							id="running_head_to_head" ${runningHeadToHeadChecked } />
					</c:otherwise>
				</c:choose>
			</div>
			...
			<div>
				<b>Performance Advancement Percentage</b> <a
				...
				<div id='PerformanceAdvancementPercentageHelp' class='help'
					...
					performance scores to be allowed to advance to the next tournament.
					X is a positive integer. <a
						href='javascript:hide("PerformanceAdvancementPercentageHelp")'>[hide]</a>
				</div>

				<input type="text" name="performance_advancement_percentage"
					id="performance_advancement_percentage"
					value="${performanceAdvancementPercentage}" class="required digits" />
