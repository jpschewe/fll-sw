...
<body>
	...
	<c:choose>
		<c:when test="${not runningHeadToHead}">
			<p>Head to head is disabled for this tournament. There is nothing
				to do here.
		</c:when>
		<c:otherwise>
			...
			<p>
				...
				tie occurs in a match; print out the score sheets again and run the
				teams a second time. Then change the scores for the teams to be the
				new scores.
			</p>
			...
			<ol>
				...
				<li>
					<form name='initialize' action='StorePlayoffParameters'
						...
						<div id='InitializeBracketHelp' class='help' style='display: none'>
							Initializing a head to head bracket allows it to be run. A head
							to head bracket cannot be initialized if any teams that are to
							compete in the head to head bracket Are still competing in a head
							to head bracket that has not been run to completion. The 3rd/4th
							place match is need if you want to know not only 1st and 2nd
							place in the bracket, but 3rd and 4th place as well. This will
							add a match with the two teams that lost in the semi-final
							matches.<a href='javascript:hide("InitializeBracketHelp")'>[hide]</a>
						</div>
				...
				<li>
					<form name='printable' action='scoregenbrackets.jsp' method='get'
						...
						<div id='ScoreGenBracketsHelp' class='help' style='display: none'>
							...
							matches to the tables specified in on the table assignment page.
							The assigned tables can be changed before printing if desired. <a
				...
				<li>
					<form name='admin' action='adminbrackets.jsp' method='get'
						...
						<div id='PrintableBracketsHelp' class='help' style='display: none'>
							...
							the emcee. The score sheet generation bracket page should be used
							to print out the first matches before this page is used to print
							the bracket. This will ensure that the assigned tables appears on
				...
				<li>
					...
					<form name="uninitialize_playoff" method="POST"
						...
						<div id='UninitializeBracketsHelp' class='help'
							...
							head bracket was initialized by mistake and a different one
							should be run first. Any scores that have been entered for this
				...
				<li>
					...
					<form name="finish_bracket" method="POST" action="FinishBracket">
						Select bracket to finish: <select id='finish-bracket'
							name='bracket'>
			...
			<c:if test="${not empty playoff_data.initializedBrackets }">
				...
				<ul>
					...
					<li><a href="remoteMain.jsp">Scrolling Head to head
							Bracket</a> (as on big screen display)<br /> Bracket and round must
					...
					<li><a href="remoteControlBrackets.jsp?scroll=false">Non-Scrolling
							Head to head Bracket</a> (as on big screen display)<br /> Bracket
						and round must be selected from the big screen display <a
						href="<c:url value='/admin/remoteControl.jsp'/>">remote
							control</a> page.</li>
