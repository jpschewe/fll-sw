...
<body>
 ...
 <c:choose>
  <c:when test="${not empty fll_user}">
   ...
   <c:choose>
    <c:when test="${all_users.size() == 1 }">
     <p>There are no users other than the one you are logged in as.
      There is nothing to do here.</p>
    </c:when>
    <c:otherwise>
     <p>Select the user to remove. You cannot remove the user that
     ...
     <form name="form.remove_user" id="form.remove_user"
      ...
      </select> <input name="submit_remove_user" id="submit_remove_user"
       value="Remove User" type="submit" />
  ...
  <c:otherwise>
   <p>
    You must be logged in as a user to ensure that we don't delete ALL
    users and then lock everyone out of the system. If you are connected
    from the server you should visit the <a
