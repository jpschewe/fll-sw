        ...
        <table border='1' id='data'>
            ...
            <c:forEach items="${JUDGES }" var="judge"
                ...
                <tr>
                    ...
                    <td>
                        <input type='text' value="${judge.id}"
                            name='id${loopStatus.count}' />
                    </td>

                    <td>
                        <select name='cat${loopStatus.count }'>
                            <c:forEach items="${CATEGORIES}" var="cat">

                                <c:choose>
                                    <c:when
                                        test="${judge.category == cat.key }">
                                        <option value="${cat.key}"
                                            selected>${cat.value }</option>
                                    </c:when>
                                    <c:otherwise>
                    ...
                    <td>
                        <select name='station${loopStatus.count }'>
                            <c:forEach items="${JUDGING_STATIONS}"
                                var="stationSel">

                                <c:choose>
                                    <c:when
                                        test="${judge.group == stationSel }">
                                        <option value="${stationSel}"
                                            selected>${stationSel }</option>
                                    </c:when>
                                    <c:otherwise>
