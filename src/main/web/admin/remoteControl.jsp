...
<body>
			...
			<tr>
				...
				<c:forEach items="${displayInformation}" var="displayInfo">
					<td>
						...
						<!-- form prefix: ${displayInfo.formParamPrefix} --> <!-- followDefault: ${displayInfo.followDefault} -->
						<!-- welcome: ${displayInfo.welcome} --> <!-- scoreboard: ${displayInfo.scoreboard} -->
						<!-- remotePageFormParamName: ${displayInfo.remotePageFormParamName} -->
						<!-- headToHead: ${displayInfo.headToHead} --> <!-- finalistSchedule: ${displayInfo.finalistSchedule} -->
						<!-- finalistTeams: ${displayInfo.finalistTeams} --> <!-- slideshow: ${displayInfo.slideshow} -->
						<!-- special: ${displayInfo.special} --> <c:choose>
							<c:when test="${displayInfo.defaultDisplay}">
          &nbsp;
          </c:when>
							<c:otherwise>
								<input type="checkbox" name="${displayInfo.deleteFormParamName}" />
							</c:otherwise>
						</c:choose>
					</td>
				</c:forEach>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${displayInfo.followDefault}">
										<input type='radio'
											name="${displayInfo.remotePageFormParamName}" value='default'
											checked />
									</c:when>
									<c:otherwise>
										<input type='radio'
					...
					<td><c:choose>
							<c:when test="${displayInfo.welcome}">
								<input type='radio' id='welcome'
									...
									value='<%=DisplayInfo.WELCOME_REMOTE_PAGE%>' checked />
							</c:when>
							<c:otherwise>
								<input type='radio' id='welcome'
									name="${displayInfo.remotePageFormParamName}"
				...
				<c:forEach items="${displayInformation}" var="displayInfo">
					<td><c:choose>
							<c:when test="${displayInfo.scoreboard}">
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
									value='<%=DisplayInfo.SCOREBOARD_REMOTE_PAGE%>' checked />
							</c:when>
							<c:otherwise>
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
			...
			<tr>
				...
				<c:forEach items="${displayInformation}" var="displayInfo">
					...
					<td><c:set var="brackets" value="${displayInfo.brackets}" />
						<input type='hidden'
						...
						value="${fn:length(brackets)}" /> <c:choose>
							<c:when test="${displayInfo.headToHead}">
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
									value='<%=DisplayInfo.HEAD_TO_HEAD_REMOTE_PAGE%>' checked />
							</c:when>
							<c:otherwise>
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
						...
						<div id="${displayInfo.formParamPrefix}bracket_selection">
							...
							<c:forEach items="${brackets}" var="bracketInfo">
								...
								<div
									...
									Bracket: <select
										...
										<c:forEach items="${divisions}" var="division">
											<c:choose>
												<c:when test="${division == bracketInfo.bracket}">
										...
										<c:forEach begin="1" end="${numPlayoffRounds}" var="numRounds">
											<c:choose>
												<c:when test="${numRounds == bracketInfo.firstRound}">
				...
				<c:forEach items="${displayInformation}" var="displayInfo">
					<td><c:choose>
							<c:when test="${displayInfo.finalistSchedule}">
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
									value='<%=DisplayInfo.FINALIST_SCHEDULE_REMOTE_PAGE%>' checked />
							</c:when>
							<c:otherwise>
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
				...
				<c:forEach items="${displayInformation}" var="displayInfo">
					<td><c:choose>
							<c:when test="${displayInfo.finalistTeams}">
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
									value='<%=DisplayInfo.FINALIST_TEAMS_REMOTE_PAGE%>' checked />
							</c:when>
							<c:otherwise>
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
			...
			<tr>
				<th>Slide show<br /> Seconds to show a slide: <input
					type='text' name='slideInterval' value="${slideShowInterval}"
					size='3' />
				</th>
					...
					<c:forEach items="${displayInformation}" var="displayInfo">
						<td><c:choose>
								<c:when test="${displayInfo.slideshow}">
									<input type='radio'
										name="${displayInfo.remotePageFormParamName}"
										value='<%=DisplayInfo.SLIDESHOW_REMOTE_PAGE%>' checked />
								</c:when>
								<c:otherwise>
									<input type='radio'
										name="${displayInfo.remotePageFormParamName}"
				...
				<c:forEach items="${displayInformation}" var="displayInfo">
					<td><c:choose>
							<c:when test="${displayInfo.special}">
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
									value='<%=DisplayInfo.SPECIAL_REMOTE_PAGE%>' checked />
							</c:when>
							<c:otherwise>
								<input type='radio'
									name="${displayInfo.remotePageFormParamName}"
						...
						</c:choose> <input type='text' name='${displayInfo.specialUrlFormParamName}'
						value="${displayInfo.specialUrl}"></td>
				</c:forEach>
	...
	<p>If you have a display that is stale. Look at the seen @ time in
		the table. If it's more than a few minutes in the past (note the time
		on the server computer). Then click the delete checkbox and then
		submit. This will remove the display name from the list and it won't
		come back until the display computer checks in with the server again;
		usually about every 30 seconds.</p>

	<p>If a display isn't updating, even after 2 minutes; walk over to
		the display computer and refresh the page with F5 or ctrl+r. Also use
		Alt-Tab (or Atl-` on some computers) to switch to the other browser
		window that has the name on it. Refresh that page also. At this point
		the display should be behaving. If it isn't, then close the browser on
		the display computer and start it again.</p>
